...
import IntentHero from "../intent/IntentHero";
import InfoGridBanner from "../condition/InfoGridBanner";
import Breadcrumbs from "../Breadcrumbs";
import IntentContent from "../intent/IntentContent";
...
const Intent: Template<TemplateRenderProps> = ({ document }: TemplateProps) => {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      ...
      >
        <div className="mt-32">
          <Breadcrumbs
            breadcrumbs={[
              ...
              {
                label: stateSlug.substring(1, 3).toUpperCase(),
                link: stateSlug,
              },
              {
                label: document.c_directoryIntentPageLink[0].name,
                link: `/${document.c_directoryIntentPageLink[0].slug}`,
              },
              { label: document.name },
            ]}
          />
        ...
        <IntentHero
          name={document.name}
          primaryPhoto={document.primaryPhoto.image.url}
        />
        <IntentContent
          name={document.name}
          ...
          conditions={
            document.taxonomy_relatedSpecialties[0].taxonomy_relatedConditions
          }
          procedures={
            document.taxonomy_relatedSpecialties[0].taxonomy_relatedProcedures
          }
        />
      </PageLayout>
    </QueryClientProvider>
